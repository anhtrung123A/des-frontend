  ...
  return (
    <main className="w-full h-full flex flex-row p-5 bg-[white] relative">
      <div className="absolute top-[15px] w-[10px]">
        ...
        <button className="px-[10px] rounded-full py-[5px] bg-[#3a3835]" onClick={()=>{expand()}}>
          <FontAwesomeIcon icon={faEllipsis} color="white">
          </FontAwesomeIcon>
        </button>
        <div className="absolute w-[200px] h-[0px] bg-slate-300 mt-[2px] rounded-[5px] transition-all duration-[500ms]" id="key-gen-dropdown">
          <div className="opacity-[0] duration-200 transition-all w-full p-[15px] flex flex-col gap-[10px]" id="key-gen-container">
            <button className="text-[14px] bg-[#3a3835] text-white px-[15px] py-[4px] rounded-full" onClick={()=> {getSecret()}}>Sinh khóa</button>
            <ul className="flex flex-col gap-[10px]">
              <li className="flex flex-col gap-[10px] items-center">
                <span>Secret Key: </span>
                <textarea name="" id="" className="flex h-[22px] rounded-[2px] resize-none px-[5px] text-[14px]" placeholder="N/A" readOnly value={secretContent}></textarea>
              </li>
            </ul>
          </div>
        </div>
            ...
            <div className="flex flex-1 flex-col gap-[10%]">
              ...
              <div className="flex flex-row justify-between">
                <div className="flex flex-row gap-[10px]">
                  <label htmlFor="file-picker" className="text-[14px] bg-[#3a3835] text-white px-[15px] py-[4px] rounded-full cursor-pointer">Tải lên</label>
          ...
          <div className="flex flex-col flex-1 gap-[10%] pb-5">
            ...
            <div className="flex flex-1 gap-[10%]">
              <div className="flex mb-10">
                <textarea name="" id="encoded-text" className="flex flex-[1] h-[200px] bg-slate-100  rounded-[10px] resize-none outline-none p-[15px]  border-black" readOnly={true} value={encryptText}></textarea>
              </div>
              <div className="flex flex-col gap-[10%]">
                <button className="text-[14px] bg-[#3a3835] text-white px-[15px] py-[4px] rounded-full" id="send" onClick={() => {send()}}>Gửi</button>
                <button className="text-[14px] bg-[#3a3835] text-white px-[15px] py-[4px] rounded-full" onClick={() => {downloadFile()}}>Lưu</button>
              </div>
              </div>
          </div>
        </div>
      ...
      <div className="h-full w-full flex flex-col gap-[14.5%]">
          <div className="flex flex-col flex-1 gap-[10%]">
            ...
            <div className="flex flex-1 flex-col gap-[10%]">
              <div className="flex mb-10">
                <textarea name="" id="received-text" onChange={(event)=>{setReceivedText(event.target.value)}} className="flex bg-slate-100 resize-none rounded-[10px] flex-1 h-[200px] outline-none p-[15px]  border-black" value={receivedText}></textarea>
              </div>
              <div className="flex flex-row justify-between">
                <div className="flex flex-row gap-[10px]">
                  <label htmlFor="file-picker-receive" className="text-[14px] bg-[#3a3835] text-white px-[15px] py-[4px] rounded-full cursor-pointer">Tải lên</label>
                  <p className="text-14px">{fileName1}</p>
                </div>
                <input type="file" className="text-[14px] hidden" id="file-picker-receive" accept=".txt, .docx"/>
                <button className="text-[14px] bg-[#3a3835] text-white px-[15px] py-[4px] rounded-full" onClick={()=>{decode()}}>Giải mã</button>
              </div>
            </div>
          </div>
          <div className="flex flex-col flex-1 gap-[10%] pb-[50px]">
            ...
            <div className="flex flex-1 gap-[10%]">
              <div className="flex mb-10">
                <textarea name="" id="decoded-text" readOnly className="flex bg-slate-100 resize-none rounded-[10px] flex-[1] h-[200px] outline-none p-[15px]  border-black" value={decryptedText}></textarea>
              </div>
              <div className="gap-5 flex flex-col">
              <div className="flex flex-row justify-between">
              </div>
                <button className="text-[14px] bg-[#3a3835] text-white rounded-full px-[15px] py-[4px]" onClick={()=>{downloadFile2()}}>Lưu</button>
              </div>
              </div>
          </div>
        </div>
